...
explore: v_events_all {
  ...
  join: v_events_all__items {
    ...
    sql: LEFT JOIN UNNEST(${v_events_all.items}) as v_events_all__items ;;
    relationship: one_to_many
  }

  join: v_events_all__event_params {
    view_label: "V Events All: Event Params"
    sql: LEFT JOIN UNNEST(${v_events_all.event_params}) as v_events_all__event_params ;;
    relationship: one_to_many
  }

  join: v_events_all__user_properties {
    view_label: "V Events All: User Properties"
    sql: LEFT JOIN UNNEST(${v_events_all.user_properties}) as v_events_all__user_properties ;;
    relationship: one_to_many
  }
}

view: v_events_all {
  sql_table_name: `prj-kmbs-p-marketing.analytics_271637361.v_events_all`
    ;;
...
view: v_events_all__items {
  ...
  dimension: v_events_all__items {
    type: string
    hidden: yes
    sql: v_events_all__items ;;
  }
}

view: v_events_all__event_params {
  dimension: key {
    type: string
    sql: key ;;
  }

  dimension: v_events_all__event_params {
    type: string
    hidden: yes
    sql: v_events_all__event_params ;;
  }
...
view: v_events_all__user_properties {
  dimension: key {
    type: string
    sql: key ;;
  }

  dimension: v_events_all__user_properties {
    type: string
    hidden: yes
    sql: v_events_all__user_properties ;;
  }
